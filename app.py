...
# ==================== 记录运动 ====================
    ...
    st.info(f"💡 预计消耗: **{estimated_cal:.0f} 千卡**")
...
# ==================== 今日统计 ====================
    ...
    with col3:
        st.metric("📈 净摄入", f"{net_calories:.0f} 千卡")
    ...
    if all_intake:
        ...
        # 简单条形图
        st.bar_chart({"蛋白质": total_protein, "脂肪": total_fat, "碳水": total_carbs})
    ...
    for meal_type, meal_name in MEAL_TYPES.items():
        ...
        if meal_cal > 0:
            meal_cals[meal_name] = meal_cal
    
    if meal_cals:
        st.bar_chart(meal_cals)
    else:
        st.info("暂无饮食记录")
...
# ==================== 周趋势 ====================
    ...
    chart_data = {
        "日期": [d['day'] for d in weekly_data],
        "摄入": [d['intake'] for d in weekly_data],
        "消耗": [d['burn'] for d in weekly_data]
    }
    
    st.subheader("摄入 vs 消耗")
    st.bar_chart(chart_data, x="日期", y=["摄入", "消耗"])
    
    st.subheader("净热量趋势")
    net_data = {"日期": [d['day'] for d in weekly_data], "净热量": [d['intake'] - d['burn'] for d in weekly_data]}
    st.line_chart(net_data, x="日期", y="净热量")
...
# ==================== 饮食建议 ====================
    ...
    with col1:
        st.metric("摄入热量", f"{total_intake:.0f} 千卡")
        st.metric("消耗热量", f"{total_burn:.0f} 千卡")
    with col2:
        st.metric("蛋白质", f"{total_protein:.1f}g")
        st.metric("脂肪", f"{total_fat:.1f}g")
        st.metric("碳水", f"{total_carbs:.1f}g")
    ...
    if total_intake == 0:
        st.info("📝 今天还没有记录饮食，快来记录吧！")
    elif total_intake < 1500:
        st.warning("⚠️ 摄入热量偏低，建议适当增加食物摄入。")
    elif total_intake > 2500:
        st.warning("⚠️ 摄入热量偏高，建议减少高热量食物。")
    else:
        st.success("✅ 摄入热量适中，继续保持！")
    
    if total_protein < 50 and total_intake > 0:
        st.info("🥩 蛋白质摄入不足，建议增加鸡胸肉、牛肉等。")
    
    if total_burn < 200 and total_intake > 0:
        st.info("🏃 今日运动量较少，建议增加30分钟运动。")
    elif total_burn >= 400:
        st.success("🎉 今日运动量充足！")
    
    st.markdown("---")
    st.subheader("🍽️ 推荐食物")
    st.write("**高蛋白低脂肪：** 鸡胸肉、鱼肉、虾仁、鸡蛋、豆腐")
    st.write("**低热量蔬菜：** 黄瓜、菠菜、西兰花、番茄")
